...
GRLIB_FuelBox_cap = round(5 * GRLIB_resources_multiplier);
...
while { GRLIB_endgame == 0 } do {
	...
	if (_active_players > 0) then {
		...
		private _FuelBox_cap = (_active_players * 3) min GRLIB_FuelBox_cap;
		...
		if (GRLIB_passive_income > 0) then {
			...
			if (_captured_military > 0) then {
				private _incomeAmmo = round (GRLIB_passive_ammount * (_captured_military min GRLIB_AmmoBox_cap) * GRLIB_resources_multiplier);
				private _incomeFuel = round (GRLIB_passive_ammount * 0.25 * (_captured_military min GRLIB_FuelBox_cap) * GRLIB_resources_multiplier);
				{
					[_x, _incomeAmmo, _incomeFuel] call ammo_add_remote_call;
				} forEach (allPlayers - (entities "HeadlessClient_F"));
				private _textAmmo = format ["Passive Income you receive %1 Ammo.", _incomeAmmo];
				private _textFuel = format ["Passive Income you receive %1 Ammo.", _incomeFuel];
				[gamelogic, _text] remoteExec ["globalChat", 0];
				diag_log format ["Passive Income %1 Ammo to all players", _incomeAmmo];
				diag_log format ["Passive Income %1 Fuel to all players", _incomeFuel];
			};
		};
